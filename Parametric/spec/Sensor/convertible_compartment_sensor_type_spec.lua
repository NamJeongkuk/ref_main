local ConvertibleCompartmentSensorType = require 'Sensor.ConvertibleCompartmentSensorType'
...
describe('convertible_compartment_sensor_type', function()
  local convertible_compartment_sensor_type = ConvertibleCompartmentSensorType(core_mock)
  ...
  it('should require all arguments', function()
    should_require_args(convertible_compartment_sensor_type, generate_config())
  end)

  it('should fail with incorrect arguments', function()
    should_fail_with('missing argument mapping_table.invalid_value', function()
      convertible_compartment_sensor_type(generate_config({
        mapping_table = {
          data = {}
        }
      }))
    ...
    should_fail_with('fresh_food_fallback_value_in_degfx100=-410000 must be in [-32768, 32767]', function()
      local config = generate_config({
        mapping_table = {
          data = { { raw = 4992, mapped = 3994 } },
          invalid_value = 30000
        },
        fresh_food_fallback_value_in_degfx100 = -410000
      })
      convertible_compartment_sensor_type(config)
    end)

    should_fail_with('freezer_fallback_value_in_degfx100=-410000 must be in [-32768, 32767]', function()
      local config = generate_config({
        mapping_table = {
          data = { { raw = 4992, mapped = 3994 } },
          invalid_value = 30000
        ...
        fresh_food_fallback_value_in_degfx100 = -500,
        freezer_fallback_value_in_degfx100 = -410000
      })
      convertible_compartment_sensor_type(config)
    end)
  ...
  it('should generate a typed string with the correct data and type convertible_compartment_sensor_type', function()
    ...
    assert(actual.is_of_type('convertible_compartment_sensor_type'))
  end)

  it('should memoize', function()
    should_memoize_calls(convertible_compartment_sensor_type, generate_config())
  end)
