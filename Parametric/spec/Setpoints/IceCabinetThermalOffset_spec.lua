local IceCabinetThermalOffset = require 'Setpoints/IceCabinetThermalOffset'
...
describe('IceCabinetThermalOffset', function()
  local ice_cabinet_thermal_offset = IceCabinetThermalOffset(core_mock)
  ...
  it('should require all arguments', function()
    should_require_args(ice_cabinet_thermal_offset, generate_config())
  end)
  ...
  it('should assert if cabinet_offset_in_degfx100 is not in range', function()
    should_fail_with('cabinet_offset_in_degfx100=32768 must be in [-32768, 32767]', function()
      ice_cabinet_thermal_offset(generate_config({
  ...
  it('should generate a typed string with the correct data and type for ice cabinet adjusted setpoint', function()
    ...
    local actual = ice_cabinet_thermal_offset(generate_config())

    assert.equals(expected, remove_whitespace(tostring(actual)))
    assert(actual.is_of_type('ice_cabinet_thermal_offset'))
  end)

  it('should memoize', function()
    should_memoize_calls(ice_cabinet_thermal_offset, generate_config())
  end)
end)
