...
*              : 25.04.2019  1.10     Added #pragma sections.
*                                     Added GCC/IAR support.
***********************************************************************************************************************/
/***********************************************************************************************************************
* Includes
***********************************************************************************************************************/
...
#if (defined(BSP_MCU_RX23_ALL) || defined(BSP_MCU_RX113))
    #pragma location=0x00007CF0U
    uint32_t dtc_vector60;
    dtc_register_data_t g_dtc_info_ctsu_wr;                    /* DTC transfer information for CTSU WR     */

    #pragma location=0x00007CF4U
    uint32_t dtc_vector61;
    dtc_register_data_t g_dtc_info_ctsu_rd;                    /* DTC transfer information for CTSU RD     */
#endif

#ifdef BSP_MCU_RX130
    #pragma location=0x000024F0U
    uint32_t dtc_vector60;
    dtc_register_data_t g_dtc_info_ctsu_wr;                    /* DTC transfer information for CTSU WR     */

    #pragma location=0x000024F4U
    uint32_t dtc_vector61;
    dtc_register_data_t g_dtc_info_ctsu_rd;                    /* DTC transfer information for CTSU RD     */
#endif
...
void dtc_ctsu_init(void)
{
    ...
    DTC.DTCST.BIT.DTCST = DTC_0_DTCST_STOP;            // do not accept DTC transfer requests
    DTC.DTCADMOD.BIT.SHORT = DTC_0_SHORT_FULL;         // use full-address mode
    ...
    g_dtc_info_ctsu_wr.MRA.bit.MD    = DTC_10_MD_BLOCK;         // Block transfer mode setting
    g_dtc_info_ctsu_wr.MRA.bit.SZ    = DTC_01_SZ_WORD;          // Word transfer setting
    g_dtc_info_ctsu_wr.MRA.bit.SM    = DTC_10_SM_INC;           // SAR value is incremented after data transfer

    g_dtc_info_ctsu_wr.MRB.bit.CHNE  = DTC_0_CHNE_DISABLE;      // Chain transfer disable setting
    g_dtc_info_ctsu_wr.MRB.bit.DISEL = DTC_0_DISEL_COMPLETED;   // Interrupt when specified data transfer is completed
    g_dtc_info_ctsu_wr.MRB.bit.DTS   = DTC_0_DTS_DESTINATION;   // Transfer destination side is repeat area or block area
    g_dtc_info_ctsu_wr.MRB.bit.DM    = DTC_10_DM_INC;           // DAR value is incremented after data xfer; src reg set to initialized array in DTC_Set_Start_of_CTSU()

    /* cast pointer to literal address */
    ...
    g_dtc_info_ctsu_rd.MRA.bit.MD    = DTC_10_MD_BLOCK;         // Block transfer mode setting
    g_dtc_info_ctsu_rd.MRA.bit.SZ    = DTC_01_SZ_WORD;          // Word transfer setting
    g_dtc_info_ctsu_rd.MRA.bit.SM    = DTC_10_SM_INC;           // SAR value is incremented after data transfer

    g_dtc_info_ctsu_rd.MRB.bit.CHNE  = DTC_0_CHNE_DISABLE;      // Chain transfer disable setting
    g_dtc_info_ctsu_rd.MRB.bit.DISEL = DTC_0_DISEL_COMPLETED;   // Interrupt when specified data transfer is completed
    g_dtc_info_ctsu_rd.MRB.bit.DTS   = DTC_1_DTS_SOURCE;        // Transfer source side is repeat area or block area
    g_dtc_info_ctsu_rd.MRB.bit.DM    = DTC_10_DM_INC;           // DAR value is incremented after data transfer

    /* cast pointer to literal address */
    g_dtc_info_ctsu_rd.SAR = (uint32_t)&CTSU.CTSUSC.WORD;   // src reg is addr of CTSUSC register; dst reg is addr of scan buf set in DTC_Set_Start_of_CTSU()
    g_dtc_info_ctsu_rd.CRA           = CTSU_DTC_RD_CNT;     // Transfer count setting (2 words)
    g_dtc_info_ctsu_rd.CRB           = 0x0000;              //

    DTC.DTCCR.BIT.RRS = DTC_1_RRS_SKIP;        // Skip setting


    /* Set addresses in DTC vector table; cast ptr to literal address */
    ...
    DTC.DTCST.BIT.DTCST = DTC_1_DTCST_START;   // DTC accept transfer requests

} /* End of function dtc_ctsu_init() */
...
void dtc_ctsu_xfer_addrs(uint32_t wr_int_src_addr, uint32_t rd_int_dst_addr)
{
    ...
    DTC.DTCCR.BIT.RRS = 1;
} /* End of function dtc_ctsu_xfer_addrs() */
