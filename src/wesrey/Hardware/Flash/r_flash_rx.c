...
***********************************************************************************************************************/
inline uint32_t R_FLASH_GetVersion (void) __attribute__ ((always_inline));
...
flash_err_t flash_lock_state (flash_states_t new_state)
{
     ...
     */
    if (flash_softwareLock(&g_bsp_Locks[BSP_LOCK_FLASH]) == true)
    {
...
void flash_release_state (void)
{
    ...
    flash_softwareUnlock(&g_bsp_Locks[BSP_LOCK_FLASH]);
}
