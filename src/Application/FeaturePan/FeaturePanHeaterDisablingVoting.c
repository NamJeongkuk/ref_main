...
static FeaturePanMode_t FeaturePanMode(FeaturePanHeaterDisablingVoting_t *instance)
{
   ...
   FeaturePanMode_t featurePanMode)
{
   if(ambientTemperature >= instance->_private.featurePanData->featurePanHeaterHighAmbientTemperatureLimitInDegFx100 &&
      featurePanMode == FeaturePanMode_ActiveHeating)
   {
...
static void OnDataModelChange(void *context, const void *_args)
{
   ...
   if(args->erd == instance->_private.config->featurePanModeErd)
   {
      const FeaturePanMode_t *featurePanMode = args->data;
      UpdateDisableHeatingVote(instance, AmbientTemperature(instance), *featurePanMode);
   }
   else if(args->erd == instance->_private.config->ambientTemperatureDegFx100Erd)
   {
      ...
      UpdateDisableHeatingVote(instance, *ambientTemperature, FeaturePanMode(instance));
   }
}
   ...
   const FeaturePanData_t *featurePanData)
{
   ...
   UpdateDisableHeatingVote(instance, AmbientTemperature(instance), FeaturePanMode(instance));
