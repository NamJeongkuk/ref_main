...
void ApplianceApiPlugin_Init(I_TinyDataSource_t *_dataSource)
{
   ...
   TinyBootLoaderParametric_ReadItem(BootLoaderImageHeader, ModelParametricItem, &modelSerial, &length);
   TinyDataSource_Write(dataSource, Erd_ModelNumber, &modelSerial);
   TinyBootLoaderParametric_ReadItem(BootLoaderImageHeader, SerialParametricItem, &modelSerial, &length);
