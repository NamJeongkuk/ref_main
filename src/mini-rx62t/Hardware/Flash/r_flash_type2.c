...
flash_err_t flash_api_close(void)
{
   ...
   flash_softwareUnlock(&g_flash_lock2);
...
***********************************************************************************************************************/
flash_err_t flash_api_blankcheck(uint32_t address, uint32_t size, flash_res_t *result) __attribute__((section(".flashCode")));
...
* Return Value : none
******************************************************************************/
void flash_access_window_set(flash_access_window_config_t *pAccessInfo) __attribute__((section(".flashCode")));
void flash_access_window_set(flash_access_window_config_t *pAccessInfo)
{
...
******************************************************************************/
flash_err_t flash_getStatus(void) __attribute__((section(".flashCode")));
...
*                    Flash is busy with another operation
******************************************************************************/
static uint8_t flash_grab_state(flash_states_t new_state) __attribute__((section(".flashCode")));
static uint8_t flash_grab_state(flash_states_t new_state)
{
   /* Check to see if lock was successfully taken */
   if(flash_softwareLock(&g_flash_lock2) == true)
   {
...
* Return Value : none
******************************************************************************/
static void flash_release_state2(void) __attribute__((section(".flashCode")));
static void flash_release_state2(void)
{
...
******************************************************************************/
static bool flash_valid_addr_check(uint32_t block_start_address) __attribute__((section(".flashCode")));
static bool flash_valid_addr_check(uint32_t block_start_address)
{
...
******************************************************************************/
static flash_err_t flash_lockbit_protection(uint32_t lock_bit) __attribute__((section(".flashCode")));
static flash_err_t flash_lockbit_protection(uint32_t lock_bit)
{
...
* Return Value : none
******************************************************************************/
void FCUIF_FIFERR(void) __attribute__((section(".flashCode"))) __attribute__((interrupt));
void FCUIF_FIFERR(void)
{
...
void FCUIF_FRDYI(void) __attribute__((section(".flashCode"))) __attribute__((interrupt));
void FCUIF_FRDYI(void)
{
   /* Local variables */
...
*                    Operation Failed
******************************************************************************/
static flash_err_t rom_write(uint32_t address, uint32_t data, uint32_t size) __attribute__((section(".flashCode")));
static flash_err_t rom_write(uint32_t address, uint32_t data, uint32_t size)
{
...
*                    Operation Failed
******************************************************************************/
static uint8_t enter_pe_mode(uint32_t flash_addr) __attribute__((section(".flashCode")));
static uint8_t enter_pe_mode(uint32_t flash_addr)
{
...
* Return Value : none
******************************************************************************/
static void exit_pe_mode(uint32_t flash_addr) __attribute__((section(".flashCode")));
static void exit_pe_mode(uint32_t flash_addr)
{
...
******************************************************************************/
uint32_t flash_get_romBlock_size(uint32_t romaddr, rom_block_info_t *pblock_info) __attribute__((section(".flashCode")));
...
******************************************************************************/
bool flash_get_romBlock_info(uint32_t block_number, rom_block_info_t *pblock_info) __attribute__((section(".flashCode")));
...
***********************************************************************************************************************/
flash_err_t flash_api_erase(flash_block_address_t block_start_address, uint32_t num_blocks) __attribute__((section(".flashCode")));
flash_err_t flash_api_erase(flash_block_address_t block_start_address, uint32_t num_blocks)
{
   ...
   uint32_t initial_block_number = 0;
...
*                    Operation Failed
******************************************************************************/
static flash_err_t flash_erase_command(FCU_BYTE_PTR const erase_addr) __attribute__((section(".flashCode")));
static flash_err_t flash_erase_command(FCU_BYTE_PTR const erase_addr)
{
...
*
******************************************************************************/
flash_err_t flash_api_write(uint32_t buffer_addr, uint32_t flash_addr, uint32_t bytes) __attribute__((section(".flashCode")));
flash_err_t flash_api_write(uint32_t buffer_addr, uint32_t flash_addr, uint32_t bytes)
{
   ...
   /* Check for ROM area */
   else if((flash_addr >= ROM_PE_ADDR) && (flash_addr < ROM_PE_ADDR + MCU_ROM_SIZE_BYTES))
   {
   ...
   while(bytes)
   {
      ...
      if(g_flash_to_flash_op == 1)
      {
         if(g_current_mode == FLD_PE_MODE)
         {
            result = data_flash_write(flash_addr, (uint32_t)&g_temp_array[0], num_byte_to_write);
         }
         else
         {
            result = rom_write(flash_addr, (uint32_t)&g_temp_array[0], num_byte_to_write);
         }
      }
      else
      {
...
*
******************************************************************************/
flash_err_t flash_api_control(flash_cmd_t cmd, void *pcfg) __attribute__((section(".flashCode")));
...
* Return Value : none
******************************************************************************/
void flash_reset(void) __attribute__((section(".flashCode")));
void flash_reset(void)
{
   /* Declare wait counter variable */
   volatile int32_t wait_cnt;
...
*                    Another flash operation is in progress
******************************************************************************/
flash_err_t flash_lockbit_program(flash_program_lockbit_config_t *lockbit_addr) __attribute__((section(".flashCode")));
flash_err_t flash_lockbit_program(flash_program_lockbit_config_t *lockbit_addr)
{
   /* Declare address pointer */
   FCU_BYTE_PTR p_addr;
...
*                    Another flash operation is in progress
******************************************************************************/
flash_err_t flash_lockbit_read(flash_read_lockbit_config_t *lockbit_info) __attribute__((section(".flashCode")));
flash_err_t flash_lockbit_read(flash_read_lockbit_config_t *lockbit_info)
{
   /* Declare address pointer */
   FCU_BYTE_PTR p_addr;
...
******************************************************************************/
static uint32_t flash_get_program_size(uint32_t bytes, uint32_t flash_addr) __attribute__((section(".flashCode")));
static uint32_t flash_get_program_size(uint32_t bytes, uint32_t flash_addr)
{
...
*                    Operation Failed
******************************************************************************/
static uint8_t notify_peripheral_clock(FCU_BYTE_PTR flash_addr) __attribute__((section(".flashCode")));
static uint8_t notify_peripheral_clock(FCU_BYTE_PTR flash_addr)
{
   /* Declare wait counter variable */
   volatile int32_t wait_cnt;
